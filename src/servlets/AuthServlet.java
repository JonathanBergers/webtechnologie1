...
public class AuthServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (m.verifyUser(username, password)) {
			...
			// maak een sessie aan, waaraan de auth filter kan zien of de
			...
			// bij forward wordt de originele methode gebruikt, de post wordt
			// dus aangeroepen.
			if (u instanceof Huurder) {
				...
				response.sendRedirect(Resources.PAGE_BEHEERDER_MAIN);
				// getServletContext().getRequestDispatcher(Resources.PAGE_BEHEERDER_MAIN).forward(request,
				// response);
